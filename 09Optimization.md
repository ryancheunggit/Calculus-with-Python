...
二维平面上有$$n$$个数据点，$$p_i=(x_i,y_i)$$，现尝试找到一条经过原点的直线$$y = ax$$，使得所有数据点到该直线的垂直误差的平方和最小。
...
要找到这样一条直线，实际上是一个优化问题，我们想要最小化的是残差（数据点和回归直线之间的水平距离）的平方和。我们所要分析的函数为：$$Err(a)=\sum_{i}{(y_i-ax_i)}^2$$，其中$$a$$为函数的输入。   
要找出该函数的最小值，首先计算一次导函数：  
