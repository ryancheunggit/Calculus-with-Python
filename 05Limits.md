...
如果$$f$$和$$g$$是连续函数，且$$\lim_{x\rightarrow a}{f(x)=0},\qquad \lim_{x\rightarrow a}{g(x)=0}$$，若$$\lim_{x\rightarrow a}{\frac{f'(x)}{g'(x)}}$$存在，则：$$\lim_{x\rightarrow a}{\frac{f(x)}{g(x)}}=\lim_{x\rightarrow a}{\frac{f'(x)}{g'(x)}}$$。    
